...
{% for id in role.members %}
    {% assign member = site.team[id] %}
    <div id="{{ member.name }}" class="authorbox position-relative py-5 px-3 mb-4 mt-4 border">   
        <div class="row">
            <div class="wrapavname col-md-3 text-center">
                {% if member.avatar %}
                <img  class="author-thumb" src="{{site.baseurl}}/assets/images/team/{{ member.avatar }}" alt="{{ author.display_name }}">
                {% else %}
